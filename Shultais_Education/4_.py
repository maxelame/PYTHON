...
Рядом с программой находится файл population.txt, в каждой строке которого хранится население России, начиная с 2003 года. То есть в первой строке население 2003 года, во второй 2004 и тд. Пример такого файла:
...
Рядом с программой находится файл population.txt с населением России за разные годы. В первой строке содержится год, с которого начали заполнять этот файл. В следующих строках хранятся непосредственно данные о населении — каждое значение с новой строки и каждое отвечает за определенный год.
...
С точки зрения компьютера, пробел — это такой же символ как и другие. Однако некоторые интерфейсы некорректно обрабатывают имена файлов содержащие пробелы. Поэтому часто в программах при сохранении файлов, пробелы в их названиях заменяют на символ подчеркивания: "файл с пробелом.txt" -> "файл_с_пробелом.txt".

Напишите программу, которая принимает первым аргументом командной строки имя файла с пробелами, а затем выводит это же имя, но с символами подчеркивания. Несколько подряд идущих пробелов нужно заменить на одно подчеркивание.
Пример использования:
...
Напишите программу, которая первым аргументом командной строки принимает строку со списком слов, разделенных запятой и пробелом, а после выводит этот же список, но в обратном порядке.
Пример использования:
> python program.py "кошки, собаки, пингвины, зебры"
...
Напишите программу, которая получает из первого аргумента командной строки один параметр — список продуктов, разделенных запятой. Программа должна вывести этот список в удобном для человека виде:
...
import sys
#принимаем строку
stroka = sys.argv[1].capitalize()
#преобразуем строку в список по разделителю ,_
spisok = stroka.split(",")

#первый элемент преобразуем снова в список и выполняем через capitalize() преобразуем к заглавой букве
#spisok[0] = spisok[0].capitalize()

# преобразуем обратно в строку с разделителем ,_ и в конце перед последним элементом разделитель И
stroka  = ", " .join(spisok[:-1])

# выводим результат
print("{} и {}." .format(stroka, str(spisok[-1])))


'''Сортировка файла

Рядом с программой находится файл products.txt со списком товаров. Каждый товар записан с новой строки. Напишите код, который прочитает список товаров из файла, отсортирует его и запишет обратно.
