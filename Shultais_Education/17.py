...
max_digit = float('-inf')
for i in range(n):
    for j in range(n): # подсчет только по заштрихованному треугольнику
        if i >= n - 1 - j:
            if matrix[i][j] >= max_digit:
                max_digit = matrix[i][j]
...
for i in range(8): # ограничили вложенный цикл областью, где могут находиться возможные ячейки с символом *
    for j in range(8):
        if abs(x - i) == abs(y - j) or x == i or y == j: # место возведения в степень мы использовали операцию abs() и сложение, так как это быстрее и проще
            matrix[i][j] = '*'
matrix[x][y] = 'Q'
for row in matrix:
    print(*row)
